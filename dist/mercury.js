...
function getExtractor(url) {
  ...
  var key = apiExtractorsKeys.concat(extractorsKeys).find(function (domain) {
    ...
    if (!domainMatch) return false; // 如果没有声明includedPaths，意味着只要域名匹配就可以

    if (!extractor.includedPaths) return true; // 匹配任意一个路径即可
...
function select(opts) {
  ...
  function selectHtml() {
    ...
    $content = transformAndClean($content); // P.S. parser 插件核心算法。根据算法提取有效内容（其实做的事情是删除所有非有效内容）
    // if (Cleaners[type]) {
    //   Cleaners[type]($content, { ...opts, defaultCleaner });
    // }

    if (allowMultiple) {
      return $content.children().toArray() // 只要有文本内容的区块。
      .filter(function (el) {
        return $(el).text().trim() !== '';
      }).map(function (el) {
        return $.html($(el));
